...
class APRDF(BaseFeaturizer):
    ...
    <https://pubs.acs.org/doi/10.1021/jp404287t>`_. In the article they describe
    the product of atomic properties as weightning of a "conventional" radiual
    ...
    def __init__(
        ...
        Args:
            ...
            Angstrom). Defaults to 20.0. lower_lim (float, optional): Lowest
            distance (in Angstrom) to consider. Defaults to 2.0. bin_size
            (float, optional): Bin size for binning. Defaults to 0.1. bw
            (Union[float, None], optional): Band width for Gaussian smearing.
                If None, the unsmeared histogram is used. Defaults to 0.1.
            properties (Tuple[str, int], optional): Properties used for
            calculation of the AP-RDF.
