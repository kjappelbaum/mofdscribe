...
def get_ase_charges(cifdata):
    ...
    ase_atoms.center(
        vacuum=0.1
    )  # move the molecule closer to the origin (it was in the center of the box for CP2K)
    return ase_atoms
...
def append_cif_molecule(ff_data, mol_cif):
    ...
    for atom in mol_ase:
        ...
        ff_data[mol_name][ff_name]["atomic_positions"].append(
            [at_label, float(atom.x), float(atom.y), float(atom.z)]
        )
    return ff_data
...
def render_ff_mixing_def(ff_data, params):
    ...
    output = ["# general rule for shifted vs truncated (file generated by aiida-raspa)", ["truncated", "shifted"][params["shifted"]], "# general rule tail corrections", ["no", "yes"][params["tail_corrections"]], "# number of defined interactions"]
...
def render_ff_def(ff_data, params, ff_mix_found):
    ...
    output = ["# rules to overwrite (file generated by aiida-raspa)", 0, "# number of defined interactions", "# mixing rules to overwrite", 0]
...
def render_molecule_def(ff_data, params, molecule_name):
    ...
    output = ["# critical constants: Temperature [T], Pressure [Pa], and Acentric factor [-] "
        + "(file generated by aiida-raspa)", ff_data[molecule_name]["critical_constants"]["tc"], ff_data[molecule_name]["critical_constants"]["pc"], ff_data[molecule_name]["critical_constants"]["af"]]
