...
class PHVect(BaseFeaturizer):
    """
    Vectorizer for Persistence Diagrams (PDs) by approximating a training set of
    persistence diagrams with Gaussian mixture models. The vectorization of a
    diagram is then the weighted maximum likelihood estimate of the mixture
    weights for the learned components given the diagram.
    ...
    def __init__(
        ...
        Args:
            ...
            compute_for_all_elements (bool, optional): If true, compute persistence images for full structure
                (i.e. with all elements). If false, it will only do it for the substructures specified with `atom_types`.
                Defaults to True.
            ...
            n_components (int, optional): The number of components or dimensions to use in the vectorized representation.
                Defaults to 20.
            apply_umap (bool, optional):  Whether to apply UMAP to the results to generate
                a low dimensional Euclidean space representation of the diagrams.
                Defaults to False.
            umap_n_components (int, optional):  The number of dimensions of euclidean space
                to use when representing the diagrams via UMAP. Defaults to 2.
            umap_metric (str, optional):  What metric to use for the UMAP embedding if ``apply_umap`` is enabled
            ...
            random_state (_type_, optional): random state propagated to the Gaussian mixture models (and UMAP).
                Defaults to None.
