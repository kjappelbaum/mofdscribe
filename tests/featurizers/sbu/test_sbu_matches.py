...
from mofdscribe.featurizers.sbu.sbu_matches import SBUMatch, match_bb
...
def test_sbu_match():
    """Ensure that our SBU matching returns reasonable results for HKUST."""
    sbu_matcher = SBUMatch()
    s = Structure.from_dict(json.loads(HKUST_node))
    feats = sbu_matcher.featurize(s)
